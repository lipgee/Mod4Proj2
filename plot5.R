...
# Find the data by year and type

NEI_vehicle <- sqldf('select year, Emissions from merge_df where type="ON-ROAD" and fips=24510 ')

NEI_year <- group_by(NEI_vehicle,year)
...
plot5 <- g + geom_point() + geom_line() + theme_bw(base_family = "Avenir", base_size = 10) + labs(x = "Year") + labs(y = "Total Emissions (tons)") + ggtitle(expression(paste("Trending of ",PM[2.5] , " Emissions from Motor Vehicle in Baltimore City (1999 - 2008)")))
ggsave(file="plot5.png", plot=plot5)
