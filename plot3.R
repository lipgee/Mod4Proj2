...
NEI_year <- group_by(NEI_Maryland,year, type)
total_em <- summarise(NEI_year, Total_Emissions=sum(Emissions))

# Plot the data and save as png  

g <- ggplot(total_em, aes(year,Total_Emissions, col=type, group=type))
plot3 <- g + geom_point() + geom_line() + theme_bw(base_family = "Avenir", base_size = 10) + labs(x = "Year") + labs(y = "Total Emissions (tons)") + ggtitle(expression(paste("Trending of ",PM[2.5] , " Emissions in Baltimore City by Source Type (1999 - 2008)")))
ggsave(file="plot3.png", plot=plot3)
